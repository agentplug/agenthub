    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, 'msg') and record.msg:
            ...
            # Tool execution
                ...
                message = message.replace(
                    '[TOOL]',
                    f"{self.COLORS['BLUE']}🔧 [TOOL]{self.COLORS['RESET']}"
                )
            ...
            # Error messages
                ...
                message = message.replace(
                    'error',
                    f"{self.COLORS['RED']}error{self.COLORS['RESET']}"
                )
                message = message.replace(
                    'failed',
                    f"{self.COLORS['RED']}failed{self.COLORS['RESET']}"
                )
            ...
            # Success messages
                ...
                message = message.replace(
                    'success',
                    f"{self.COLORS['GREEN']}success{self.COLORS['RESET']}"
                )
                message = message.replace(
                    'completed',
