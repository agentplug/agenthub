...
class LoggingManager:
    ...
    def _get_settings_from_config(self) -> LoggingSettings:
        ...
        try:
            ...
            return LoggingSettings(
                ...
                format="structured",
                enable_file_logging=os.getenv("AGENTMANAGER_ENABLE_FILE_LOGGING", "false").lower() == "true",
                ...
                suppress_urllib3=os.getenv("AGENTMANAGER_SUPPRESS_URLLIB3", "true").lower() == "true",
                enable_colors=os.getenv("AGENTMANAGER_ENABLE_COLORS", "true").lower() == "true"
            )
        except Exception:
            ...
            return LoggingSettings(
                ...
                suppress_http=os.getenv("AGENTMANAGER_SUPPRESS_HTTP", "true").lower() == "true",
                suppress_urllib3=os.getenv("AGENTMANAGER_SUPPRESS_URLLIB3", "true").lower() == "true",
                enable_colors=os.getenv("AGENTMANAGER_ENABLE_COLORS", "true").lower() == "true"
            )
    ...
    def _setup_console_handler(self, log_level: int) -> None:
        ...
        if self.settings.format == "structured":
            ...
            formatter = ColorfulFormatter(
                '%(message)s' if self.settings.quiet_mode else '%(asctime)s - %(levelname)s - %(message)s',
                enable_colors=self.settings.enable_colors
            )
    ...
    def _configure_library_loggers(self) -> None:
        ...
        if self.settings.quiet_mode:
            ...
            logging.getLogger('agentmanager').setLevel(logging.INFO)
            ...
            for logger_name in ['mcp', 'mcp.client', 'mcp.client.session', 'mcp.client.stdio']:
    ...
    def get_logger(self, name: str) -> logging.Logger:
        ...
        Args:
            name: Logger name (will be prefixed with 'agentmanager.')
