...
from agenthub.core.common.result import Result, ErrorType, agent_not_found_error, validation_error, execution_error
from agenthub.core.agents import AgentLoader, AgentWrapper
from agenthub.core.tools import get_tool_registry, assign_tools_to_agent
...
class UnifiedAgentLoader:
    ...
    def __init__(self):
        ...
        self.runtime.process_manager.use_dynamic_execution = not self.config.use_subprocess_execution
        if self.config.default_timeout:
            self.runtime.process_manager.timeout = self.config.default_timeout
    
    def load(self, agent_name: str, options: Optional[AgentLoadOptions] = None) -> Result[AgentWrapper]:
        """
        ...
        try:
            ...
            if "/" not in agent_name:
                return Result.fail(validation_error(
                    ...
                    {"agent_name": agent_name, "expected_format": "namespace/agent"}
                ))
            ...
            if not self.storage.agent_exists(namespace, agent):
                if options.auto_install:
                    logger.info(f"Agent '{agent_name}' not found. Installing automatically...")
                    install_result = self._install_agent(agent_name, options.setup_environment)
                    if install_result.is_err():
                        return install_result
                    logger.info(f"Agent '{agent_name}' installed successfully!")
                else:
                    return Result.fail(agent_not_found_error(
                        agent_name,
                        {"namespace": namespace, "agent": agent, "auto_install": False}
                    ))
            ...
            try:
                ...
                return Result.fail(execution_error(
            ...
            if options.use_subprocess is not None:
                self.runtime.process_manager.use_dynamic_execution = not options.use_subprocess
            ...
            if options.tools:
                validation_result = self._validate_and_assign_tools(agent_id, options.tools)
                if validation_result.is_err():
                    return validation_result
            
            logger.info(f"Successfully loaded agent '{agent_name}' with {len(options.tools or [])} tools")
        ...
        try:
            ...
            if not result.success:
                return Result.fail(execution_error(
                    ...
                    {"agent_name": agent_name, "installer_error": result.error_message}
                ))
    ...
    def _validate_and_assign_tools(self, agent_id: str, tools: List[str]) -> Result[None]:
...
_loader: Optional[UnifiedAgentLoader] = None
