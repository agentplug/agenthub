...
from ..tools import get_tool_registry, ToolNotFoundError, ToolAccessDeniedError
from ...monitoring.metrics import record_tool_execution, record_metric
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not self.tool_registry.can_agent_access_tool(self.agent_id, tool_name):
                raise ToolAccessDeniedError(f"Agent '{self.agent_id}' does not have access to tool '{tool_name}'")
            ...
            record_tool_execution(tool_name, self.agent_id, execution_time, success, error_type)
            record_metric("tool_execution_time", execution_time, {"tool_name": tool_name, "agent_id": self.agent_id})
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    logger.warning(f"Tool execution attempt {attempt + 1} failed for {tool_name}: {e}. Retrying...")
                    await asyncio.sleep(retry_delay * (2 ** attempt))  # Exponential backoff
                else:
                    logger.error(f"Tool execution failed after {max_retries + 1} attempts for {tool_name}: {e}")
        
        # If we get here, all retries failed
        raise last_error
    
    async def execute_multiple_tools(self, tool_requests: List[Dict[str, Any]]) -> List[str]:
        ...
        for result in results:
            if isinstance(result, Exception):
                processed_results.append(f'{{"error": "Tool execution failed: {str(result)}"}}')
            else:
