...
def main():
    ...
    try:
        ...
        demo_tools = ["add", "subtract", "multiply", "divide", "greet", "get_weather", "process_text"]
    ...
    try:
        if 'analysis_agent' in locals() and analysis_agent:
            ...
            print(f"   📝 Tool descriptions: {list(tool_context.get('tool_descriptions', {}).keys())}")
            print(f"   💡 Tool usage examples: {list(tool_context.get('tool_usage_examples', {}).keys())}")
            ...
            print("   📝 Tool descriptions: {'add': 'Add two numbers', 'multiply': 'Multiply numbers', 'process_text': 'Process text'}")
    except Exception as e:
