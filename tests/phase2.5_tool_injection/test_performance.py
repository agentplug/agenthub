...
class TestToolInjectionPerformance:
    ...
    def test_tool_registration_performance(self):
        ...
        assert registration_time < 5.0, f"Tool registration took {registration_time:.2f} seconds"
    ...
    def test_tool_metadata_retrieval_performance(self):
        ...
        assert retrieval_time < 0.5, f"Metadata retrieval took {retrieval_time:.2f} seconds"
    ...
    def test_concurrent_tool_registration_performance(self):
        ...
        def register_tool(tool_id: int):
            try:
                @tool(name=f"concurrent_perf_tool_{tool_id}", description=f"Concurrent tool {tool_id}")
        ...
        assert concurrent_time < 2.0, f"Concurrent registration took {concurrent_time:.2f} seconds"
    ...
    def test_concurrent_tool_execution_performance(self):
        ...
        def execute_tool(tool_id: int):
            try:
                result = self.registry.execute_tool("concurrent_exec_tool", {"value": tool_id})
        ...
        # Should complete in reasonable time (less than 2 seconds)
        assert concurrent_time < 2.0, f"Concurrent execution took {concurrent_time:.2f} seconds"
    ...
    def test_memory_usage_stability(self):
        ...
        for _ in range(10):
            for i in range(200):
                result = self.registry.execute_tool(f"memory_tool_{i}", {"param": f"test_{i}"})
    ...
    def test_agent_tool_assignment_performance(self):
        ...
        # Should complete in reasonable time (less than 1 second)
        assert assignment_time < 1.0, f"Tool assignment took {assignment_time:.2f} seconds"
    ...
    def test_tool_context_generation_performance(self):
        ...
        for i in range(50):
            @tool(name=f"context_tool_{i}", description=f"Context tool {i} with complex metadata")
            def context_tool(
                param1: str = f"default1_{i}",
                param2: int = i,
                param3: bool = True
            ) -> dict:
        ...
        # Should complete in reasonable time (less than 2 seconds)
        assert context_time < 2.0, f"Tool context generation took {context_time:.2f} seconds"
    ...
    def test_benchmark_tool_registration_scalability(self):
        ...
        times_per_tool = [reg_time / tool_count for tool_count, reg_time in registration_times]
        ...
        assert max_time_per_tool / min_time_per_tool < 10, "Tool registration scalability is poor"
