...
class TestCompleteInstallationWorkflow:
    ...
    def test_complete_workflow_success_with_environment(self, temp_storage_path, mock_successful_clone, 
                                                      mock_successful_validation, mock_successful_environment_setup,
        ...
        with patch('agentmanager.github.auto_installer.ENVIRONMENT_AVAILABLE', True):
            with patch('agentmanager.github.auto_installer.EnvironmentSetup') as mock_env_class:
                # Mock environment setup instance
                mock_env = Mock()
                mock_env.setup_environment.return_value = mock_successful_environment_setup
                mock_env.install_dependencies.return_value = mock_successful_dependency_install
                mock_env_class.return_value = mock_env
                
                # Create installer with environment setup enabled
                installer = AutoInstaller(base_storage_path=temp_storage_path, setup_environment=True)
                
                # Mock the underlying components
                with patch.object(installer.repository_cloner, 'clone_agent', return_value=mock_successful_clone):
    ...
    def test_complete_workflow_success_without_environment(self, temp_storage_path, mock_successful_clone, 
        ...
        installer = AutoInstaller(base_storage_path=temp_storage_path, setup_environment=False)
    ...
    def test_workflow_failure_at_clone_step(self, temp_storage_path):
        ...
        with patch.object(installer.repository_cloner, 'clone_agent', return_value=mock_clone_failure):
            result = installer.install_agent("test/agent")
    ...
    def test_workflow_failure_at_validation_step(self, temp_storage_path, mock_successful_clone):
        ...
        with patch.object(installer.repository_cloner, 'clone_agent', return_value=mock_successful_clone):
            with patch.object(installer.repository_validator, 'validate_repository', return_value=mock_validation_failure):
                result = installer.install_agent("test/agent")
    ...
    def test_workflow_failure_at_dependency_step(self, temp_storage_path, mock_successful_clone, 
                                               mock_successful_validation, mock_successful_environment_setup):
    ...
    def test_workflow_with_environment_not_available(self, temp_storage_path, mock_successful_clone, 
                                                   mock_successful_validation):
        """Test workflow when environment setup is requested but not available."""
        # Mock environment setup as not available
        with patch('agentmanager.github.auto_installer.ENVIRONMENT_AVAILABLE', False):
            installer = AutoInstaller(base_storage_path=temp_storage_path, setup_environment=True)
    ...
    def test_workflow_performance_metrics(self, temp_storage_path, mock_successful_clone, 
                                        mock_successful_validation):
    ...
    def test_workflow_next_steps_guidance(self, temp_storage_path, mock_successful_clone, 
                                         mock_successful_validation):
...
class TestWorkflowIntegrationWithRealComponents:
    ...
    def test_storage_path_integration(self, real_installer, temp_storage_path):
        ...
        custom_installer = AutoInstaller(base_storage_path=custom_path, setup_environment=False)
