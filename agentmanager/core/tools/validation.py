            ...
            try:
                ...
                for param in signature.parameters.values():
                    if param.kind == param.VAR_POSITIONAL:
                        ...
                        warnings.append(
                            "Function uses **kwargs - may complicate parameter validation")
                ...
                if unannotated_params:
                    warnings.append(
                        f"Parameters without type annotations: {', '.join(unannotated_params)}")
                ...
                try:
                    ...
                    if not security_result.approved:
                        if security_result.level == SecurityLevel.UNSAFE:
                            errors.extend(
                                [f"Security risk: {risk}" for risk in security_result.risks])
                        else:
                            warnings.extend(
                                [f"Security concern: {risk}" for risk in security_result.risks])
