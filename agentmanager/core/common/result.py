...
class Result(Generic[T]):
    ...
    @classmethod
    def fail_with(cls, error_type: ErrorType, message: str, details: Optional[dict] = None, cause: Optional[Exception] = None) -> "Result[T]":
...
def agent_not_found_error(agent_name: str, details: Optional[dict] = None) -> Error:
    """Create an agent not found error."""
    return Error(ErrorType.AGENT_NOT_FOUND, f"Agent not found: {agent_name}", details)


def execution_error(message: str, details: Optional[dict] = None, cause: Optional[Exception] = None) -> Error:
    """Create an execution error."""
    return Error(ErrorType.EXECUTION_ERROR, message, details, cause)


def tool_error(message: str, details: Optional[dict] = None, cause: Optional[Exception] = None) -> Error:
    """Create a tool error."""
    return Error(ErrorType.TOOL_ERROR, message, details, cause)


def timeout_error(timeout: int, details: Optional[dict] = None) -> Error:
    """Create a timeout error."""
    return Error(ErrorType.TIMEOUT_ERROR, f"Operation timed out after {timeout} seconds", details)


def permission_error(message: str, details: Optional[dict] = None) -> Error:
