...
from agentmanager.core.tools import get_tool_registry, ToolNotFoundError, ToolAccessDeniedError
...
class AgentToolManager:
        ...
        async with self._client_lock:
            if self.client is None:
                ...
                server_params = StdioServerParameters(
                    command="python",
                    args=["-c", "from agentmanager.core.tools import get_mcp_server; import asyncio; asyncio.run(get_mcp_server().run_stdio())"]
                )
    ...
    async def execute_tool(self, agent_id: str, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        if not self.has_tool_access(agent_id, tool_name):
            raise ToolAccessDeniedError(f"Agent '{agent_id}' does not have access to tool '{tool_name}'")
...
# Global instance
_tool_manager: Optional[AgentToolManager] = None
