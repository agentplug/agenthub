...
from agentmanager.core.mcp.agent_tool_manager import get_tool_manager
...
class ToolInjector:
    ...
    def inject_tools_into_agent_context(self, agent_id: str, tool_names: List[str]) -> Dict[str, Any]:
        ...
        system_prompt = self._create_tool_system_prompt(assigned_tools, tool_descriptions, tool_parameters, tool_examples)
    ...
    def _create_tool_system_prompt(self, tool_names: List[str], descriptions: Dict[str, str], 
                                 parameters: Dict[str, Dict], examples: Dict[str, Dict]) -> str:
        ...
        for tool_name in tool_names:
            ...
            prompt_parts.append(f"Description: {descriptions.get(tool_name, 'No description available')}")
            
            if tool_name in parameters:
                param_info = []
                for param_name, param_type in parameters[tool_name].items():
                    param_info.append(f"  - {param_name}: {param_type.__name__ if hasattr(param_type, '__name__') else str(param_type)}")
...
# Global instance
_tool_injector: Optional[ToolInjector] = None
